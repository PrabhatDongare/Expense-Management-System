...
const HomePage = () => {
  ...
  useEffect(() => {
    const getAllTransactions = async () => {
      try {
        ...
        setLoading(true);
        const res = await axios.post(`${backendUrl}/api/v1/transections/get-transection`, {
          userid: user._id,
          frequency,
          selectedDate,
          type,
  ...
  const handleDelete = async (record) => {
    try {
      setLoading(true);
      await axios.post(`${backendUrl}/api/v1/transections/delete-transection`, {
  ...
  const handleSubmit = async (values) => {
    try {
      ...
      if (editable) {
        ...
        await axios.post(`${backendUrl}/api/v1/transections/add-transection`, {
          ...values,
          userid: user._id,
        });
        setLoading(false);
